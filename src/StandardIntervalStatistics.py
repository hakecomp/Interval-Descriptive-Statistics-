# Copyright 2018 Aline Brum Loreto<aline.loreto@inf.ufpel.edu.br>, Alice Fonseca Finger <aliceffinger@gmail.com>, Mauricio Dorneles
...
'''
Details about source of methods describes below
...
from sys import *
from intStatistics.utils.initInterval import *

__all__ = ["StandardIntervalStatistics"]

class StandardIntervalStatistics(object):
	...
	def average(intervalList):
		'''
		Average Interval defined for Standard Interval Arithmetic. 
		
		>>> from intStatistics import StandardIntervalStatistics as si
		>>> si.average([IReal(2.0),IReal(4.3)])
		[3.15 , 3.15]
		'''
	...
	def median(intervalList):
		'''
		Median Interval defined for Standard Interval Arithmetic. 
		
		>>> from intStatistics import StandardIntervalStatistics as si
		>>> si.median([IReal(2.0),IReal(4.3),IReal(3.0)])
		[3.0 , 3.0]
		'''
		lowers = []
		uppers = []
	...
	@staticmethod
	def range(intervalList):
		'''
		Range Interval defined for Standard Interval Arithmetic. 
		
		>>> from intStatistics import StandardIntervalStatistics as si
		>>> si.range([IReal(2.0),IReal(4.3)])
		[2.3,2.3]
		'''
		AmpTotal = IReal(0)
		lowers = []
	...
	def variance(intervalList):
		...
		variance = IReal(0)
		for i in range(len(intervalList)):
			variance += (powI((intervalList[i] - StandardIntervalStatistics.average(intervalList)), 2))
	...
	def stdeviation(intervalList):
		standardDeviation = sqrtI(StandardIntervalStatistics.variance(intervalList))
	...
	def coefVariance(intervalList):
		...
		coefVariance = StandardIntervalStatistics.stdeviation(intervalList)/StandardIntervalStatistics.average(intervalList)
		return coefVariance
	...
	def coVariance(intervalListOne, intervalListTwo):
		...
		AverageX = StandardIntervalStatistics.average(intervalListOne)
		AverageY = StandardIntervalStatistics.average(intervalListTwo)
		a = []
	...
	def coefCorrelation(intervalListOne, intervalListTwo):
		coefCorrelation = StandardIntervalStatistics.coVariance(intervalListOne, intervalListTwo)/(StandardIntervalStatistics.stdeviation(intervalListOne)*StandardIntervalStatistics.stdeviation(intervalListTwo))
